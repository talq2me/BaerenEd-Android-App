...
class GameEngine(
    ...
    fun submitAnswer(userAnswers: List<String>): Boolean {
        ...
        if (isCorrect) {
            correctCount++
            android.util.Log.d("GameEngine", "Answer submitted - correct, correctCount: $correctCount/${config.requiredCorrectAnswers}")
        } else {
            incorrectCount++
            android.util.Log.d("GameEngine", "Answer submitted - incorrect, incorrectCount: $incorrectCount")
        }

        // Advance after every submission so each answered question counts toward total.
        currentIndex++
