...
-- Requires af_stars_to_minutes (af_stars_to_minutes.sql). Identifies item by p_item_label (key in checklist_items JSONB).
-- Call: POST /rest/v1/rpc/af_update_checklist_item with body e.g.
--   {"p_profile": "TE", "p_item_label": "Laundry", "p_done": true}

CREATE OR REPLACE FUNCTION af_update_checklist_item(
  p_profile text,
  p_item_label text,
  p_done boolean
)
...
BEGIN
  ...
  IF p_done AND NOT old_done THEN
    ...
    IF item_stars > 0 THEN
      ...
      add_mins := af_stars_to_minutes(item_stars);
    END IF;
  END IF;
...
GRANT EXECUTE ON FUNCTION af_update_checklist_item(text, text, boolean) TO anon, authenticated, service_role;
