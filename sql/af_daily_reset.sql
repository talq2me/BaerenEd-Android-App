...
-- Call this before reading user_data so the row for the given profile with last_reset date not equal to today (EST)
-- gets reset: blank required_tasks, checklist_items, practice_tasks, berries_earned, banked_mins, chores;
-- set last_reset and last_updated to now() EST. Does not change coins_earned, pokemon_unlocked, game_indices.
...
BEGIN
  ...
  UPDATE user_data
  SET
    last_reset = (NOW() AT TIME ZONE 'America/New_York'),
